...
import (
	"context"
	"fmt"
	"go-otel-demo/service-c/internal/telemetry"
	"time"
...
func (c *Client) BrokenOperation(ctx context.Context) (resultErr error) {
	...
	err := bad.Get(ctx, "otel-demo-chain-broken-key").Err()
	if err == nil {
...
func (c *Client) SlowOperation(ctx context.Context, key, value string, delay time.Duration) (string, error) {
	tracer := otel.Tracer("go-otel-demo/service-c/redis")
	ctx, span := tracer.Start(ctx, "redis synthetic slow operation")
	defer span.End()

	time.Sleep(delay)
	return c.SetGet(ctx, key, value)
}
