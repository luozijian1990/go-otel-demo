...
def clean_trace(raw: Dict[str, Any], slow_threshold_ms: int = 3000) -> Dict[str, Any]:
    ...
    for span in sorted(trace.get("spans") or [], key=lambda item: item.get("startTime", 0)):
        ...
        parent_id = parent_span_id([r for r in references if r.get("traceID", trace.get("traceID")) == trace.get("traceID")])
    ...
    ids = {s["span_id"] for s in cleaned_spans}
    limitations = []
    for parent in cleaned_spans:
        if any(r.get("refType") != "CHILD_OF" or r.get("traceID", trace.get("traceID")) != trace.get("traceID") for r in parent["references"]):
            limitations.append("non-parent or cross-trace references retained but excluded from exclusive time")
        start = parent["start_time"] or 0
        end = start + parent["duration_ms"] * 1000
        intervals = []
        if parent["parent_span_id"] and parent["parent_span_id"] not in ids:
            limitations.append("missing parent: " + str(parent["parent_span_id"]))
        for child in cleaned_spans:
            if child["parent_span_id"] != parent["span_id"]:
                continue
            a = child["start_time"] or 0
            b = a + child["duration_ms"] * 1000
            if a < start or b > end:
                limitations.append("child interval outside parent; clock skew or incomplete trace")
            if min(b, end) > max(a, start):
                intervals.append((max(a, start), min(b, end)))
        covered, cursor = 0, start
        for a, b in sorted(intervals):
            covered += max(0, b - max(a, cursor))
            cursor = max(cursor, b)
        parent["exclusive_ms"] = round(max(0, end - start - covered) / 1000, 3)
    ...
    return {
        ...
        "blocking_candidates": sorted(cleaned_spans, key=lambda s: s["exclusive_ms"], reverse=True)[:5],
    }
...
def clean_exception_logs(logs: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for log in logs:
        ...
        if any(k.startswith("exception.") for k in fields) or fields.get("event") in ("exception", "error"):
            fields["timestamp_us"] = log.get("timestamp")
