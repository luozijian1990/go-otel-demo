...
def main():
    p=argparse.ArgumentParser();p.add_argument('--base-url',default='http://localhost:18086');p.add_argument('--controls',action='store_true');a=p.parse_args()
    ...
    if a.controls:
        ...
        key=next(line.split('=',1)[1].strip() for line in Path('.env').read_text().splitlines() if line.startswith('DEMO_FAULT_SECRET='))
        ...
        assert status==500 and '1146' in body['error'],(status,body)
    ...
    for port in (18083,18086):
        with urllib.request.urlopen('http://localhost:'+str(port)+'/demo/runs',timeout=5) as r:assert r.status==200
